...
function normalizeMotionScriptName(name) {
  return (name || "motion-script")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 64);
...
export function analyzeMotionScriptText(text, sourceUrl = "") {
  ...
  const addRecipe = (name, trigger, properties = [], confidence = "medium", evidence = "") => {
    const key = `${name}|${trigger}`;
    if (recipeKeys.has(key)) return;
    recipeKeys.add(key);
    recipes.push({ name, trigger, properties, confidence, evidence });
  };
  ...
  const libraryMatchers = [
    ...
    ["lenis", /\blenis\b/],
    ["animejs", /\banime\b/],
    ["framer-motion", /\bframer\b|\bmotion\b/],
    ["threejs", /\bthree\b/],
    ["barba", /\bbarba\b/],
  ];
  libraryMatchers.forEach(([name, pattern]) => {
    if (pattern.test(lower)) libraries.add(name);
  });

  if (/requestanimationframe/.test(lower)) runtimeSignals.add("script:requestAnimationFrame");
  if (/timeline/.test(lower)) runtimeSignals.add("script:timeline");
  if (/preloader|loader|splash|intro/.test(lower)) runtimeSignals.add("script:preloader");
  if (/scrolltrigger|onscroll|addEventlistener\(['"]scroll/.test(lower)) runtimeSignals.add("script:scroll");

  const durationMatches = content.match(/\b\d+(?:\.\d+)?(?:ms|s)\b/gi) || [];
  durationMatches.slice(0, 40).forEach((value) => durations.add(value.toLowerCase()));

  const easingMatches = content.match(/\b(?:linear|ease(?:-in|-out|-in-out)?)\b/gi) || [];
  easingMatches.slice(0, 40).forEach((value) => easings.add(value.toLowerCase()));

  if (/(?:from|to)\s*\(|opacity/.test(lower)) addRecipe("fade", "enter", ["opacity"], "medium", "opacity references in script");
  if (/transform|translate|scale|rotate/.test(lower)) addRecipe("hover-lift", "hover", ["transform"], "medium", "transform references in script");
  if (/scrolltrigger|onscroll|addeventlistener\(['"]scroll/.test(lower)) {
    addRecipe("scroll-timeline", "scroll", ["transform", "opacity"], "high", "scroll timeline hooks in script");
  }
  if (/preloader|loader|splash|intro/.test(lower)) {
    addRecipe("preloader-sequence", "load", ["opacity", "transform"], "high", "preloader markers in script");
  }
  if (/lenis|locomotive/.test(lower)) {
    addRecipe("smooth-scroll", "scroll", ["transform"], "high", "smooth scrolling library markers in script");
  }

  const inferredName =
    /preloader|loader/.test(lower) ? "preloader-sequence"
      : /scrolltrigger|onscroll/.test(lower) ? "scroll-timeline-sequence"
        : /lenis|locomotive/.test(lower) ? "smooth-scroll-sequence"
          : /timeline/.test(lower) ? "timeline-sequence"
            : sourceUrl ? sourceUrl.split("/").pop()?.split("?")[0] || "motion-script" : "motion-script";

  const shouldCapture =
    libraries.size > 0 ||
    runtimeSignals.size > 0 ||
    recipes.length > 0 ||
    /@keyframes|transition|animation/.test(lower);

  return {
    name: normalizeMotionScriptName(inferredName),
    libraries: Array.from(libraries).slice(0, 20),
    runtimeSignals: Array.from(runtimeSignals).slice(0, 20),
    durations: Array.from(durations).slice(0, 20),
    easings: Array.from(easings).slice(0, 20),
...
export async function extractBranding(
  ...
  try {
    ...
    const [
      ...
      extractLayoutSemantics(page),
      extractMotionPatterns(page),
      extractMediaEmbeds(page),
    ]);
    ...
    console.log(bordersTotal > 0 ?
      chalk.hex('#50FA7B')(`  ✓ Borders: ${borders?.combinations?.length || 0} combinations`) :
    ...
    const totalInputs = (inputs?.text?.length || 0) + (inputs?.checkbox?.length || 0) + (inputs?.radio?.length || 0) + (inputs?.select?.length || 0);
    console.log(totalInputs > 0 ? chalk.hex('#50FA7B')(`  ✓ Inputs: ${totalInputs} styles`) : chalk.hex('#FFB86C')(`  ⚠ Inputs: 0 styles`));
    ...
    console.log(layoutSemantics?.sections?.length > 0 ? chalk.hex('#50FA7B')(`  ✓ Layout sections: ${layoutSemantics.sections.length}`) : chalk.hex('#FFB86C')(`  ⚠ Layout sections: 0`));
    console.log(motion?.recipes?.length > 0 ? chalk.hex('#50FA7B')(`  ✓ Motion recipes: ${motion.recipes.length}`) : chalk.hex('#FFB86C')(`  ⚠ Motion recipes: 0`));
    console.log(media?.items?.length > 0 ? chalk.hex('#50FA7B')(`  ✓ Media embeds: ${media.items.length}`) : chalk.hex('#FFB86C')(`  ⚠ Media embeds: 0`));
    console.log();
  ...
  return await page.evaluate(() => {
    ...
    els.forEach((el) => {
      ...
      seen.set(key, {
        ...
        letterSpacing: letterSpacing !== "normal" ? letterSpacing : null,
    ...
    return {
      ...
      sources: {
        ...
        fontDisplay: sources.fontDisplay,
      },
    };
  });
}
...
/**
 * Extract page layout semantics for builder-oriented reconstruction.
 */
async function extractLayoutSemantics(page) {
  return await page.evaluate(() => {
    const sections = [];
    const landmarks = ["header", "main", "section", "footer", "nav", "aside"];

    landmarks.forEach((tag) => {
      const count = document.querySelectorAll(tag).length;
      if (count > 0) {
        sections.push({ type: tag, count, confidence: count > 1 ? "high" : "medium" });
      }
    });

    const semanticSelectors = [
      { type: "hero", selector: "[class*='hero'], [data-hero]" },
      { type: "features", selector: "[class*='feature'], [id*='feature']" },
      { type: "pricing", selector: "[class*='pricing'], [id*='pricing']" },
      { type: "testimonials", selector: "[class*='testimonial'], [id*='testimonial']" },
      { type: "faq", selector: "[class*='faq'], [id*='faq']" },
      { type: "cta", selector: "[class*='cta'], [data-cta]" },
    ];

    semanticSelectors.forEach(({ type, selector }) => {
      const count = document.querySelectorAll(selector).length;
      if (count > 0) {
        sections.push({ type, count, confidence: count > 1 ? "high" : "medium" });
      }
    });

    const gridEvidence = document.querySelectorAll(
      "[class*='container'], [class*='grid'], [class*='col-'], [class*='row'], [class*='gutter']"
    ).length;
...
/**
 * Extract reusable motion patterns from computed styles and keyframe names.
 */
async function extractMotionPatterns(page) {
  const baseMotion = await page.evaluate(() => {
    ...
    function addRecipe(name, trigger, props, confidence = "medium", evidence = "") {
      const key = `${name}|${trigger}`;
      if (recipeSet.has(key)) return;
      recipeSet.add(key);
      recipes.push({
        name,
        trigger,
    ...
    function inferScriptName(text) {
      if (!text) return "animation-script";
      const lower = text.toLowerCase();
      if (lower.includes("preloader") || lower.includes("loader")) return "preloader-sequence";
      if (lower.includes("scrolltrigger")) return "scroll-trigger-sequence";
      if (lower.includes("lenis") || lower.includes("locomotive")) return "smooth-scroll-sequence";
      if (lower.includes("timeline")) return "timeline-sequence";
      if (lower.includes("requestanimationframe")) return "raf-loop-sequence";
      if (lower.includes("gsap")) return "gsap-sequence";
      return "animation-script";
    }

    function detectLibrariesFromText(text, source = "script") {
      const lower = (text || "").toLowerCase();
      if (lower.includes("gsap")) libraries.add("gsap");
      if (lower.includes("scrolltrigger")) libraries.add("scrolltrigger");
      if (lower.includes("lenis")) libraries.add("lenis");
      if (lower.includes("locomotive")) libraries.add("locomotive-scroll");
      if (lower.includes("anime")) libraries.add("animejs");
      if (lower.includes("framer")) libraries.add("framer-motion");
      if (lower.includes("three")) libraries.add("threejs");
      if (lower.includes("barba")) libraries.add("barba");

      if (lower.includes("requestanimationframe")) runtimeSignals.add(`${source}:requestAnimationFrame`);
      if (lower.includes("timeline")) runtimeSignals.add(`${source}:timeline`);
      if (lower.includes("preloader") || lower.includes("loader")) runtimeSignals.add(`${source}:preloader`);
    }

    function addPreloaderCandidate(el, reason = "selector") {
      const computed = getComputedStyle(el);
      const rect = el.getBoundingClientRect();
      ...
      const key = `${Math.round(rect.width)}x${Math.round(rect.height)}|${computed.position}|${reason}`;
      if (preloaderMap.has(key)) return;
      preloaderMap.set(key, {
        selectorHint: `${el.tagName.toLowerCase()}${el.id ? `#${el.id}` : ""}`,
        reason,
        position: computed.position,
        zIndex: computed.zIndex,
        width: Math.round(rect.width),
        height: Math.round(rect.height),
        animationName: computed.animationName !== "none" ? computed.animationName : null,
        transitionDuration: computed.transitionDuration || null,
      });
    }

    function evaluateOverlayCandidate(el) {
      const computed = getComputedStyle(el);
      const rect = el.getBoundingClientRect();
      ...
      const hasProgressNode = !!el.querySelector("[class*='progress'], [class*='loader'], [class*='percent'], progress");
      const hasAnimation = computed.animationName && computed.animationName !== "none";
      const reason = hasProgressNode
        ? "overlay-progress"
        : hasAnimation
          ? "overlay-animation"
          : zIndex >= 30
      ...
      addPreloaderCandidate(el, reason);
    }

    document.querySelectorAll("*").forEach((el) => {
      const computed = getComputedStyle(el);
      const classText = (typeof el.className === "string" ? el.className : el.className?.baseVal || "").toLowerCase();
      const idText = (el.id || "").toLowerCase();
      const attrText = `${classText} ${idText} ${(el.getAttribute("data-testid") || "").toLowerCase()} ${(el.getAttribute("aria-label") || "").toLowerCase()}`;

      if (computed.transitionDuration && computed.transitionDuration !== "0s") {
        computed.transitionDuration.split(",").map((d) => d.trim()).forEach((d) => durations.add(d));
      }
      if (computed.transitionTimingFunction) {
        computed.transitionTimingFunction.split(",").map((e) => e.trim()).forEach((e) => easings.add(e));
      }
      if (computed.transitionProperty && computed.transitionProperty !== "all") {
        computed.transitionProperty.split(",").map((p) => p.trim()).forEach((p) => properties.add(p));
      }
      if (computed.animationName && computed.animationName !== "none") {
        computed.animationName.split(",").map((k) => k.trim()).forEach((k) => keyframes.add(k));
      }

      if (attrText.includes("preload") || attrText.includes("loader") || attrText.includes("splash") || attrText.includes("intro")) {
        addPreloaderCandidate(el, "name-match");
      }

      if (el.tagName === "DIV" || el.tagName === "SECTION" || el.tagName === "ASIDE") {
        evaluateOverlayCandidate(el);
      }
    });

    if (properties.has("transform") || properties.has("box-shadow")) {
      addRecipe(
        "hover-lift",
        "hover",
        ["transform", "box-shadow"].filter((p) => properties.has(p)),
        "high",
        "transform/box-shadow transition detected"
      );
    }
    if (properties.has("opacity")) {
      addRecipe("fade", "enter", ["opacity"], "high", "opacity transition detected");
    }
    if (properties.has("filter")) {
      addRecipe("filter-shift", "hover", ["filter"], "medium", "filter transition detected");
    }
    if (properties.has("background-color") || properties.has("color")) {
      addRecipe(
        "color-morph",
        "hover",
        ["background-color", "color"].filter((p) => properties.has(p)),
        "medium",
        "color transitions detected"
      );
    }

    if (keyframes.size > 0) {
      addRecipe("keyframe-animation", "timeline", ["transform", "opacity"], "medium", "keyframes detected");
    }

    // Parse local stylesheets to infer keyframes, hover/scroll intent and motion hints.
    try {
      for (const sheet of Array.from(document.styleSheets || [])) {
        ...
        for (const rule of rules) {
          ...
          if (text.includes("@keyframes") && rule.name) {
            keyframes.add(rule.name);
          }

          if (selector.includes(":hover")) {
            runtimeSignals.add("css:hover");
          }
          if (selector.includes(":focus")) {
            runtimeSignals.add("css:focus");
          }
          if (selector.includes("sticky")) {
            runtimeSignals.add("css:sticky");
          }
          if (selector.includes("preload") || selector.includes("loader")) {
            runtimeSignals.add("css:preloader");
          }
        }
      }
    } catch (error) {
      // Ignore stylesheet access failures.
    }

    // Detect runtime libraries exposed on window
    const globalChecks = [
      ["gsap", () => typeof window.gsap !== "undefined"],
      ["scrolltrigger", () => typeof window.ScrollTrigger !== "undefined"],
      ["lenis", () => typeof window.Lenis !== "undefined" || typeof window.lenisVersion !== "undefined"],
      ["animejs", () => typeof window.anime !== "undefined"],
      ["framer-motion", () => typeof window.Motion !== "undefined" || typeof window.framerMotion !== "undefined"],
      ["threejs", () => typeof window.THREE !== "undefined"],
    ];
    globalChecks.forEach(([name, checker]) => {
      try {
        if (checker()) libraries.add(name);
      } catch (error) {
        // Ignore runtime check failures.
      }
    });

    // Detect script references and inline script evidence for animation systems.
    ...
    scripts.forEach((script, index) => {
      ...
      if (src) {
        ...
        detectLibrariesFromText(lowerUrl, "src");
        scriptReferences.push({ type: "external", url: absoluteUrl });
        if (SCRIPT_KEYWORDS.some((kw) => lowerUrl.includes(kw))) {
          const guessedName = normalizeScriptName(
            absoluteUrl.split("/").pop()?.split("?")[0] || `external-motion-${index + 1}`
          );
          animationScripts.push({
            name: guessedName || `external-motion-${index + 1}`,
            type: "external",
            url: absoluteUrl,
            source: "script-src",
          });
      ...
      const lower = inlineContent.toLowerCase();
      detectLibrariesFromText(lower, "inline");
      if (SCRIPT_KEYWORDS.some((kw) => lower.includes(kw))) {
        const inferred = inferScriptName(lower);
        const trimmed = inlineContent.slice(0, 120000);
        animationScripts.push({
          name: `${inferred}-${index + 1}`,
          type: "inline",
          source: "inline-script",
          content: trimmed,
          truncated: inlineContent.length > trimmed.length,
        });
      }
    });

    if (libraries.has("gsap") && (libraries.has("scrolltrigger") || runtimeSignals.has("src:scrolltrigger"))) {
      addRecipe("scroll-timeline", "scroll", ["transform", "opacity"], "high", "GSAP + ScrollTrigger detected");
    }
    if (libraries.has("lenis") || runtimeSignals.has("src:lenis")) {
      addRecipe("smooth-scroll", "scroll", ["transform"], "high", "Lenis or smooth-scroll library detected");
    }
    if (preloaderMap.size > 0 || runtimeSignals.has("css:preloader") || runtimeSignals.has("inline:preloader")) {
      addRecipe("preloader-sequence", "load", ["opacity", "transform"], "high", "preloader overlay evidence detected");
    }

    try {
      const htmlOverflow = getComputedStyle(document.documentElement).overflow;
      const bodyOverflow = getComputedStyle(document.body).overflow;
      if ((htmlOverflow === "hidden" || bodyOverflow === "hidden") && preloaderMap.size > 0) {
        runtimeSignals.add("dom:overflow-hidden-with-preloader");
      }
    } catch {
      // Ignore overflow read failures.
    }

    const dedupScripts = [];
    const scriptSeen = new Set();
    animationScripts.forEach((item, index) => {
      const key = item.url || `${item.name}|${item.content?.slice(0, 200) || ""}`;
      if (scriptSeen.has(key)) return;
      scriptSeen.add(key);
      dedupScripts.push({
        ...item,
        name: normalizeScriptName(item.name) || `animation-script-${index + 1}`,
      });
    });

    return {
      durations: Array.from(durations).slice(0, 12),
      easings: Array.from(easings).slice(0, 12),
      properties: Array.from(properties).slice(0, 20),
      keyframes: Array.from(keyframes).slice(0, 20),
      libraries: Array.from(libraries).slice(0, 20),
      runtimeSignals: Array.from(runtimeSignals).slice(0, 30),
      preloaders: Array.from(preloaderMap.values()).slice(0, 10),
      scriptReferences: scriptReferences.slice(0, 80),
      animationScripts: dedupScripts.slice(0, 50),
      recipes,
    };
  });

  const mergedDurations = new Set(baseMotion.durations || []);
  const mergedEasings = new Set(baseMotion.easings || []);
  const mergedLibraries = new Set(baseMotion.libraries || []);
  const mergedSignals = new Set(baseMotion.runtimeSignals || []);
  const mergedRecipes = [...(baseMotion.recipes || [])];
  const recipeKeys = new Set(mergedRecipes.map((r) => `${r?.name || "recipe"}|${r?.trigger || "unknown"}`));
  const animationScripts = [...(baseMotion.animationScripts || [])];
  const scriptKeys = new Set(animationScripts.map((s) => s?.url || `${s?.name || "script"}|${s?.content?.slice(0, 200) || ""}`));
  ...
  const firstPartyScriptUrls = (baseMotion.scriptReferences || [])
    .filter((ref) => ref?.url && /^https?:\/\//.test(ref.url))
    .map((ref) => ref.url)
    .filter((url, index, arr) => arr.indexOf(url) === index)
    .filter((url) => {
      try {
        const parsed = new URL(url);
        return parsed.origin === currentOrigin && parsed.pathname.toLowerCase().endsWith(".js");
      } catch {
        return false;
      }
    })
  ...
  for (const scriptUrl of firstPartyScriptUrls) {
    try {
      const response = await page.context().request.get(scriptUrl, { timeout: 8000 });
      if (!response.ok()) continue;
      const contentType = (response.headers()["content-type"] || "").toLowerCase();
      if (contentType && !contentType.includes("javascript") && !contentType.includes("text/plain")) continue;

      const scriptText = await response.text();
      const analysis = analyzeMotionScriptText(scriptText, scriptUrl);
      analysis.libraries.forEach((item) => mergedLibraries.add(item));
      analysis.runtimeSignals.forEach((item) => mergedSignals.add(item));
      analysis.durations.forEach((item) => mergedDurations.add(item));
      analysis.easings.forEach((item) => mergedEasings.add(item));
      analysis.recipes.forEach((recipe) => {
        const key = `${recipe?.name || "recipe"}|${recipe?.trigger || "unknown"}`;
        if (recipeKeys.has(key)) return;
        recipeKeys.add(key);
      ...
      if (analysis.shouldCapture) {
        const snippet = scriptText.slice(0, 120000);
        const key = `${scriptUrl}|${snippet.slice(0, 200)}`;
        if (!scriptKeys.has(key)) {
          scriptKeys.add(key);
          animationScripts.push({
            name: analysis.name,
            type: "external",
            source: "fetched-script",
            url: scriptUrl,
            content: snippet,
            truncated: scriptText.length > snippet.length,
          });
        }
      }
    } catch {
      // Non-fatal: script fetch can fail due to anti-bot/CORS/network.
    }
  }

  return {
    ...baseMotion,
    durations: Array.from(mergedDurations).slice(0, 20),
    easings: Array.from(mergedEasings).slice(0, 20),
    libraries: Array.from(mergedLibraries).slice(0, 20),
    runtimeSignals: Array.from(mergedSignals).slice(0, 40),
    recipes: mergedRecipes.slice(0, 60),
    animationScripts: animationScripts.slice(0, 60),
  };
}

/**
 * Detect media and embed usage as component-level references.
 */
async function extractMediaEmbeds(page) {
  return await page.evaluate(() => {
    const items = [];
    ...
    document.querySelectorAll("iframe").forEach((iframe) => {
      ...
      items.push({
        type: "iframe",
        src,
        provider: src.includes("youtube") ? "youtube" : src.includes("vimeo") ? "vimeo" : "unknown",
    ...
    const carouselCount = carouselSelectors.reduce(
