...
function mergeMotionFromPages(rawPages = []) {
  ...
  const seenPreloaders = new Set();
  const seenScripts = new Set();
  const seenRefs = new Set();

  for (const page of rawPages) {
    const motion = page?.extraction?.motion;
    if (!motion) continue;

    (motion.durations || []).forEach((item) => durations.add(item));
    (motion.easings || []).forEach((item) => easings.add(item));
    (motion.properties || []).forEach((item) => properties.add(item));
    (motion.keyframes || []).forEach((item) => keyframes.add(item));
    (motion.libraries || []).forEach((item) => libraries.add(item));
    (motion.runtimeSignals || []).forEach((item) => runtimeSignals.add(item));

    (motion.recipes || []).forEach((recipe) => {
    ...
    (motion.preloaders || []).forEach((preloader) => {
      const key = `${preloader?.selectorHint || "el"}|${preloader?.width || 0}|${preloader?.height || 0}|${preloader?.reason || "unknown"}`;
      if (seenPreloaders.has(key)) return;
      seenPreloaders.add(key);
      preloaders.push(preloader);
    });

    (motion.scriptReferences || []).forEach((ref) => {
      const key = `${ref?.type || "ref"}|${ref?.url || ""}`;
      if (seenRefs.has(key)) return;
      seenRefs.add(key);
      scriptReferences.push(ref);
    });

    (motion.animationScripts || []).forEach((script) => {
      const key = script?.url || `${script?.name || "script"}|${script?.content?.slice(0, 200) || ""}`;
      if (seenScripts.has(key)) return;
      seenScripts.add(key);
      animationScripts.push(script);
    });
  }
  ...
  return {
    ...
    properties: Array.from(properties).slice(0, 40),
    keyframes: Array.from(keyframes).slice(0, 40),
    libraries: Array.from(libraries).slice(0, 20),
    runtimeSignals: Array.from(runtimeSignals).slice(0, 40),
    recipes: recipes.slice(0, 60),
    preloaders: preloaders.slice(0, 20),
    scriptReferences: scriptReferences.slice(0, 150),
    animationScripts: animationScripts.slice(0, 120),
  };
}

function extractMotionFromResult(result, rawPages = []) {
  const merged = mergeMotionFromPages(rawPages);
  if (merged.recipes.length > 0 || merged.animationScripts.length > 0 || merged.preloaders.length > 0) {
    return merged;
  }
...
function extractAssetsFromResult(result) {
  ...
  (result.motion?.animationScripts || []).forEach((script) => {
    if (script?.url) {
      refs.push({
        type: "motion-script",
        url: script.url,
...
export async function extractBundle({
  ...
  const bundle = {
    ...
    motion: options.includeMotion ? extractMotionFromResult(baseResult, perPage) : { durations: [], easings: [], recipes: [] },
...
export async function saveBundleOutput({
  ...
  if (bundle.motion && Array.isArray(bundle.motion.animationScripts) && bundle.motion.animationScripts.length > 0) {
    const motionScriptsDir = join(outputDir, "motion", "scripts");
    mkdirSync(motionScriptsDir, { recursive: true });
    const scriptFiles = [];
    let scriptCounter = 0;

    for (const script of bundle.motion.animationScripts) {
      if (script && typeof script.content === "string" && script.content.length > 0) {
        scriptCounter += 1;
        const name = sanitizeFileName(script.name || `inline-animation-${scriptCounter}`, `inline-animation-${scriptCounter}`);
        const fileName = `${String(scriptCounter).padStart(2, "0")}-${name}.js`;
        const filePath = join(motionScriptsDir, fileName);
        writeFileSync(filePath, script.content);
        scriptFiles.push({
          kind: "inline",
          name: script.name || name,
          path: filePath,
          bytes: Buffer.byteLength(script.content, "utf-8"),
        });
        continue;
      }

      if (script && script.url && /^https?:\/\//.test(script.url)) {
        scriptCounter += 1;
        const host = new URL(script.url).hostname.toLowerCase();
        if (Array.isArray(allowlistHosts) && allowlistHosts.length > 0 && !allowlistHosts.includes(host)) {
          scriptFiles.push({
            kind: "external",
            name: script.name || `external-animation-${scriptCounter}`,
            url: script.url,
            path: null,
            skipped: true,
        ...
        const name = sanitizeFileName(script.name || `external-animation-${scriptCounter}`, `external-animation-${scriptCounter}`);
        const fileName = `${String(scriptCounter).padStart(2, "0")}-${name}.js`;
        const filePath = join(motionScriptsDir, fileName);
        try {
          const meta = await downloadAssetWithLimits(script.url, filePath, maxAssetBytes);
          scriptFiles.push({
            kind: "external",
            name: script.name || name,
            url: script.url,
    ...
    bundle.motion.scriptFiles = scriptFiles;
    writeFileSync(join(outputDir, "motion.json"), JSON.stringify(bundle.motion, null, 2));
  }
