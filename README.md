...
dembrandt bmw.de --output-bundle   # Save full extraction bundle (raw/tokens/components/layout/motion/report)
dembrandt bmw.de --max-pages 12 --max-depth 3 --concurrency 3 # Multi-page crawl limits
dembrandt bmw.de --strict-security # Enforce strict SSRF/DNS protections
```
...
Use `--output-bundle` to generate:
- `raw/pages/*.raw.json`
- `tokens.light.json` and `tokens.dark.json`
- `components.json`, `layout.json`, `motion.json`, `assets.json`
- `summary.json` (simple highlights + summary tokens for animation/colors/borders/paddings)
- `dtcg.tokens.json` and `report.json`
...
For production-like local web mode (single server, serves built UI + API):
...
The web UI can also trigger extraction directly and let users download generated files (`summary.json`, `tokens.light.json`, `tokens.dark.json`, `components.json`, `layout.json`, `motion.json`, `assets.json`, `dtcg.tokens.json`, `report.json`).

## Portainer (Local)

Deploy with stack YAML from this repo:
